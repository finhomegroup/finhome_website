...
//
// SOURCES, added 2026-09-16. This was the row where the omission bit
// hardest: a vendored IRS table named in four places with no href anywhere,
// which is exactly what `components/calc/calculator-page.tsx` means by
// "naming a decree and a date in prose is not a citation a reader can
// check". Both hrefs were fetched on 2026-09-16 and their content read.
//
// Pub 590-B (2025 edition) is the table's home, and its worked example uses
// the age-75 divisor 24,6 — which is the same figure this header already
// recorded for the 1960 cohort, so the vendored table and the published one
// agree at the one point both state. The RMD topics page carries the formula
// and both penalty rates verbatim.
//
// ONE COPY SHARPENING came out of that reading. The page said the 25%
// penalty drops to 10% "nếu được sửa trong thời hạn quy định" — true but
// unquantified, on the one figure a reader would act on. IRS states the
// window as two years, so the label, the notice and formula.body[5] now all
// say 2 years. The label was also the tightest of the three surfaces
// (a card label, ~30 characters), which is why it reads "trong 2 năm".
//
...
export const US_RMD = {
  ...
  form: {
    ...
    correctedLabel: "Nếu sửa trong 2 năm, còn 10%",
    ...
    shortfallNotice:
      "Mức dự định rút của bạn thấp hơn mức bắt buộc. Phần thiếu chịu thuế phạt 25%, giảm còn 10% nếu được sửa trong thời hạn quy định — IRS ghi thời hạn đó là trong vòng 2 năm, nghĩa là rút bù phần còn thiếu và khai mẫu trong khoảng đó. Đáng chú ý là khoảng thiếu ở đây rất nhỏ: làm tròn khoản rút xuống một con số chẵn cho gọn là cách phổ biến nhất để bị phạt, vì mức bắt buộc gần như không bao giờ là số chẵn.",
    metNotice:
  ...
  formula: {
    ...
    body: [
      ...
      "Phần thiếu so với mức bắt buộc chịu thuế phạt 25%, giảm còn 10% nếu được sửa trong vòng 2 năm. Mức phạt này từng là 50% và được SECURE 2.0 hạ xuống. Số tiền phạt tính trên phần thiếu, không phải trên toàn bộ khoản phải rút.",
  ...
  sources: {
    title: "Nguồn",
    intro:
      "Hai trang dưới đây là căn cứ cho bảng hệ số mà công cụ kèm sẵn, cho tuổi bắt buộc bắt đầu rút, và cho hai mức thuế phạt. Bảng hệ số là dữ liệu duy nhất trang này không lấy từ ô nhập của bạn, nên nó là thứ cần kiểm chứng được nhất. Danh sách chỉ gồm nguồn cho các quy tắc trang này dùng, không gồm quy định về tài khoản thừa kế hay Bảng Tuổi thọ Chung — hai thứ trang này nói rõ là không mô hình hóa.",
    items: [
      {
        url: "https://www.irs.gov/publications/p590b",
        label: "IRS Publication 590-B — Bảng Tuổi thọ Thống nhất (Appendix B, Table III)",
        note: "Nguồn của bảng hệ số chia vendored trong công cụ. Bản đối chiếu ngày 16/09/2026 là ấn bản cho năm thuế 2025 và ví dụ của IRS dùng hệ số 24,6 cho tuổi 75 — đúng con số công cụ trả về cho người sinh năm 1960 ở tuổi bắt buộc của họ.",
      },
      {
        url: "https://www.irs.gov/retirement-plans/plan-participant-employee/retirement-topics-required-minimum-distributions-rmds",
        label: "IRS — Mức rút tối thiểu bắt buộc: cách tính và tiền phạt",
        note: "Căn cứ cho công thức và cho hai mức phạt: mức bắt buộc là số dư cuối năm dương lịch liền trước chia cho kỳ phân phối lấy từ Bảng Tuổi thọ Thống nhất, và phần chưa rút chịu “a 25% excise tax on the amount not distributed as required (10% if withdrawn within 2 years)”. Cũng là nơi nêu Roth IRA không phải rút tối thiểu khi chủ tài khoản còn sống.",
      },
    ],
  },
} as const;
